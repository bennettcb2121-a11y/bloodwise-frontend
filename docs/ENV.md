...
| `CLARION_UNLOCK_CODES` | Comma-separated one-time codes for **free Clarion analysis** unlock (e.g. `BETA2025,FRIEND01`). Grants `analysis_purchased_at` only — user still needs to subscribe for recurring Clarion+ features. Requires `unlock_redemptions` table (see `supabase/migrations/007_unlock_codes.sql`). |
| `CLARION_FAMILY_UNLOCK_CODES` | Comma-separated one-time codes for **full friends-and-family comps** (e.g. `FAM-MOM-01,FAM-DAD-01`). Grants analysis access **and** sets `plan_tier='full'`, so the recipient gets Clarion+ features without ever hitting Stripe. Same `unlock_redemptions` table; codes are one-time-use per code string. |
